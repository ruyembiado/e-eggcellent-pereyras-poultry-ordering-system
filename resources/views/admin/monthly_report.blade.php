		...
		<div class="table-responsive">
			<table class="table table-bordered" width="100%" cellspacing="0">
				...
				<tbody>
					...
					@php
					$firstWeekOfMonth = \Carbon\Carbon::create($selected_year, $selected_month, 1)->weekOfYear;
					$weekInMonth = $week['week_number'] - $firstWeekOfMonth + 1;
					@endphp
					<tr>
						<td>Week {{ $weekInMonth }}</td>
					<td>{{ $week['products'] }}</td>
					<td>{{ $week['quantity'] }}</td>
...
@foreach ($report as $week)
[
"Week {{ $week['week_number'] - \Carbon\Carbon::create($selected_year, $selected_month, 1)->weekOfYear + 1 }}",
"{{ $week['products'] }}",
"{{ $week['quantity'] }}",
"₱{{ number_format($week['total'], 2) }}"
],
@endforeach
[
"Grand Total",
