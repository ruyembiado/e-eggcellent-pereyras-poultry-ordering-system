		...
		<div class="table-responsive">
			<table class="table table-bordered" width="100%" cellspacing="0">
				...
				<tbody>
					@if ($report->isEmpty())
					<tr>
						<td colspan="5" class="text-center">No data available for this week.</td>
					</tr>
					@else
...
[
"{{ $dayName }}",
"{{ collect($dayOrders)->pluck('products')->implode(', ') }}",
"{{ collect($dayOrders)->sum('quantity') }}",
"₱{{ number_format(collect($dayOrders)->sum('total'), 2) }}"
],
@endforeach
@endforeach
[
...
XLSX.writeFile(workbook, "weekly_report_week{{ $selected_week }}_{{ $month_name }}_{{ $selected_year }}.xlsx");
}
</script>
